...
import sys
import json
sheet_name="Portfolio Express"
# df = pd.read_excel("C:\\Users\\ShailendraSingh\\Desktop\\ameri.xlsx",sheet_name=sheet_name)

# import module
import openpyxl

# wrkbk = openpyxl.load_workbook("C:\\Users\\ShailendraSingh\\Desktop\\ameri_1.xlsx")
n = 13
# sh = wrkbk.active
def portfolioExpress(wrkbk,version):
    ...
    def cal_term(value):
        ...
        product_info_dict["productType"] = prod_packing[-2]
    ...
    for i in range(1, sh.max_row + 1):
        ...
        for j in range(1, sh.max_column + 1):
            ...
            if cell_obj.font.b and cell_obj.value!= None  :
                ...
                def isfloat(num):
                    try:
                        ...
                        return False
                if isfloat(cell_obj.value) ==False :
            ...
            if i == bold_indx + 1 and bold_flag == True and ((sh.cell(row =bold_indx+ 1, column = j)).value)!=None and  ((sh.cell(row =bold_indx+ 1, column = j)).font.b) == False:
        ...
        for row_range  in range(row_val['start'],row_val['end']+1):
            ...
            for col_range in range(col_val['start'],col_val['end']+1):
                ...
                if row_range > row_val['start']:
                    print(row_range,(sh.cell(row=row_range, column=col_val['start'])).value)
                    day=(sh.cell(row=row_val["start"], column=col_range)).value
                    rate_val = (sh.cell(row=row_range, column=col_val['start'])).value
                    print(rate_val)
                    json_list_obj["rate"] =copy.deepcopy(rate_val)
                    ...
                    if " Day" in day:
                        day = day.replace(" Day",'')
                        data_value["day"]=copy.deepcopy(int(day))
                    elif "Day" in day:
                        day = day.replace("Day", '')
                        data_value["day"] = copy.deepcopy(int(day))
                    elif " day" in day:
                        ...
                        data_value["day"] = copy.deepcopy(day)
                ...
                if row_range > row_val['start'] and col_range > col_val['start']:
                    ...
                    data_value["bondRate"] =cell_obj.value
    ...
    with open("PortfoliloExpress.json","w") as j_file:
